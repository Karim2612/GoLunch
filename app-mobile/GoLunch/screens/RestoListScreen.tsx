...
export default function RestoListScreen({ navigation }) {
  ...
  const getRestaurants = async () => {
    try {
      const response = await fetch(api.GETRESTO);
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            <TouchableOpacity style={styles.card} activeOpacity={0.2} onPress={() => {
              //Sam: change navigate to when press a fav restaurant
        ...
        <View style={styles.cardImage}>
          ...
          <View style={{ flex: 1 }}>
            ...
            style={styles.cardLocation}>{item.localisation.ville}</Text> 
